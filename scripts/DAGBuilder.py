...
class DAGBuilder:
    @staticmethod
    def buildDAGGPT3(df, verbose=False, model="text-davinci-002"):
        G = nx.DiGraph()
        for c in df.columns:
            G.add_node(c)
        nodes = list(G.nodes)
        for pair in itertools.permutations(nodes, r=2):
            if "Unnamed" in pair[0] or "Unnamed" in pair[1]:
                continue
            ans_parsed, ans = DAGBuilder.getCausEffect(
                pair[0], pair[1], verbose, model=model
            )
            if ans_parsed == "YES":
    ...
    def pruneEdges(G, df):
        ...
        for u, v in G.edges():
            ...
            for p in itertools.chain.from_iterable(
                itertools.combinations(potential, r) for r in range(1, 3)
            ):
                if u in p or v in p:
                    continue
    ...
    @staticmethod
    def getCausEffect(c, e, verbose=False, model="text-davinci-002"):
        prompt = (
